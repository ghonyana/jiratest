...
class CloudWatchJsonFormatter(JsonFormatter):
    ...
    def add_fields(self, log_record: Dict[str, Any], record: logging.LogRecord, message_dict: Dict[str, Any]) -> None:
        ...
        log_record["timestamp"] = datetime.now(UTC).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + "Z"
        ...
        optional_fields = ["event_id", "fingerprint", "jira_issue_key", "action", "duration_ms", "error_type"]
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if any(pattern in message_lower for pattern in sensitive_patterns):
            ...
            # Apply Authorization: Bearer pattern first (most specific)
            message = re.sub(r"(authorization:\s*bearer\s+)[\w\-\.=_]+", r"\1[REDACTED]", message, flags=re.IGNORECASE)
            # Apply key=value patterns (don't match standalone bearer)
...
def setup_logging(level: str = "INFO", environment: str = "production") -> None:
    ...
    formatter = CloudWatchJsonFormatter(service="error-triage", environment=environment)
    ...
    root_logger.info(
        ...
        extra={"action": "logging_initialized", "log_level": log_level, "environment": environment},
    )
