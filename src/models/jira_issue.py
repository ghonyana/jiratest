...
class JiraIssueData:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.summary) > 255:
            raise ValueError(f"Summary exceeds maximum length of 255 characters: {len(self.summary)}")

        # Validate priority
        if self.priority not in ALLOWED_PRIORITIES:
            raise ValueError(f"Priority must be one of {ALLOWED_PRIORITIES}, got: {self.priority}")

        # Validate severity
        if self.severity not in ALLOWED_SEVERITIES:
            raise ValueError(f"Severity must be one of {ALLOWED_SEVERITIES}, got: {self.severity}")
