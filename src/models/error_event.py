...
class NormalizedErrorEvent:
    ...
    def __post_init__(self) -> None:  # noqa: C901
        ...
        if self.source not in ["vercel", "gcp"]:
            raise ValueError(f"Invalid source '{self.source}'. Must be 'vercel' or 'gcp'.")
        ...
        for field_name, field_value in required_string_fields.items():
            if not isinstance(field_value, str):
                raise ValueError(f"Field '{field_name}' must be a string, got {type(field_value).__name__}.")
        ...
        if not isinstance(self.occurred_at, datetime):
            raise ValueError(
                f"Field 'occurred_at' must be a datetime instance, " f"got {type(self.occurred_at).__name__}."
            )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "NormalizedErrorEvent":
        ...
        if isinstance(occurred_at_str, str):
            ...
            raise ValueError(
                f"Field 'occurred_at' must be ISO format string or datetime, " f"got {type(occurred_at_str).__name__}."
            )
