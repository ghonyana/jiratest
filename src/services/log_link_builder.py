...
class LogLinkBuilder:
    ...
    def build_vercel_link(self, deployment_url: str, trace_id: str) -> str:
        ...
        if not trace_id or not isinstance(trace_id, str) or not trace_id.strip():
            logger.error(
                "build_vercel_link called with empty or invalid trace_id", extra={"deployment_url": deployment_url}
            )
        ...
        query_params = {"deploymentUrl": deployment_url, "q": f"traceId:{trace_id}"}
        ...
        logger.info(
            "Built Vercel log link", extra={"deployment_url": deployment_url, "trace_id": trace_id, "link": full_url}
        )
    ...
    def build_gcp_link(self, project: str, insert_id: str) -> str:
        ...
        if not insert_id or not isinstance(insert_id, str) or not insert_id.strip():
            logger.error("build_gcp_link called with empty or invalid insert_id", extra={"project": project})
        ...
        logger.info("Built GCP log link", extra={"project": project, "insert_id": insert_id, "link": full_url})
